...
public class ArrayUtils {
    ...
    */
    ...
    public static <T> T readFromConsole(String name, java.ru.vsu.cs.ru.vsu.cs.util.function.Function<String, T> converter) {
        Scanner scanner = new Scanner(System.in);
        while (true) {
            try {
    ...
    private static <T> T readArrayFromConsole(String arrName, java.ru.vsu.cs.ru.vsu.cs.util.function.Function<String, T> strToArrConverter) {
    ...
    */
    ...
    public static Object[][] toObjectArray2(String[] lines) {
        Object[][] arr2 = new Object[lines.length][];
        for (int r = 0; r < lines.length; r++) {
            arr2[r] = toObjectArray(lines[r]);
        }
        return arr2;
    }

    private static Object[] toObjectArray(String str) {
        ...
        while (scan.hasNext()) {
            ...
            try {
                Integer intValue = Integer.valueOf(current);
                items.add(intValue);
            } catch (NumberFormatException e1) {
                try {
                    Double doubleValue = Double.valueOf(current);
                    items.add(doubleValue);
                } catch (NumberFormatException e2) {
                    if (current.length() == 1) {
                        items.add(current.charAt(0));
                    } else {
                        items.add(current);
                    }
                }
    ...
    public static Object[][] readObjectArray2FromFile(String fileName) {
        try {
            return toObjectArray2(readLinesFromFile(fileName));
        } catch (FileNotFoundException e) {
            return null;
        }
    }

    public static Object[] readObjectArrayFromFile(String fileName) {
        try (Scanner scanner = new Scanner(new File(fileName), "UTF-8")) {
            scanner.useLocale(Locale.ROOT);

            // Читаем первую строку файла
            if (!scanner.hasNextLine()) {
                return new Object[0]; // Возвращаем пустой массив, если файл пуст
            }
            String line = scanner.nextLine();

            // Разбиваем строку на элементы
            scanner.close(); // Закрываем первый сканер
            ...
            while (lineScanner.hasNext()) {
                if (lineScanner.hasNextInt()) {
                    ...
                    list.add(lineScanner.nextBoolean());
                } else {
